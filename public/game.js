...
export class game extends React.Component {
	constructor(props) {
		...
		if (this.state.time_control === "hourglass") {
			this.state["timer_" + (this.state.turn === "red" ? "black" : "red")] = this.state["timer_" + (this.state.turn === "red" ? "black" : "red")] + val
		}
		...
		this.state.interval = setInterval(() => {
			...
			if (!this.props.preview && !this.state.game_ended && this.state.timer_red > 0 && this.state.timer_black > 0)
				this.setState({
					["timer_" + this.state.turn]: !this.state.branched_off ? (this.state["timer_" + this.state.turn] - 0.017).toFixed(3) : this.state["timer_" + this.state.turn],
					["timer_" + (this.state.turn === "red" ? "black" : "red")]: !this.state.branched_off && this.state.time_control === "hourglass" ? (this.state["timer_" + (this.state.turn === "red" ? "black" : "red")] + 0.017).toFixed(3) : this.state["timer_" + (this.state.turn === "red" ? "black" : "red")]
				})
			else
				this.setState({
	...
	game_update_handler(data) {
		...
		if (this.state.moves_counter === 0 || (this.state.turn === "red" && data.a[2] + data.a[3] === "rd") || (this.state.turn === "black" && data.a[2] + data.a[3] === "bd")) {
			...
			if (this.props.offline && this.props.red.username != "AI" && this.props.black.username != "AI" && this.state.ghost_cards.length === 0)
	...
	rightclick_event(card_right_clicked, stackname) {
		...
		if (card_right_clicked) {
			...
			if (this.dragstart_event(card_right_clicked, true)) {
				...
				this.setState({
					ghost_dragging: card_right_clicked
				})
				...
				onMouseDown: e => {
					if (e.buttons === 4 || e.buttons === 1) {
						if (this.state.ghost_cards.length > 0) {
							...
							this.setState(game_from_PON(this.state.game_states[this.state.replay_step + 1]), () => {
								var val = (new Date() - this.state.last_action) / 1000
								this.setState({["timer_" + this.state.turn] : this.state["timer_" + this.state.turn] - val})
								if (this.state.time_control === "hourglass") {
									this.setState({["timer_" + (this.state.turn === "red" ? "black" : "red")] : this.state["timer_" + (this.state.turn === "red" ? "black" : "red")] + val})
								}
							})
							if (this.state.replay_step > -1) {
